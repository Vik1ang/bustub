...
namespace bustub {
/**
 * TrieNode is a generic container for any in Trie
 */
class TrieNode {
 public:
  /**
 ...
 */
template <typename T>
class TriNodeWithValue : public TrieNode {
 private:
 ...
 public:
   ...
   * @param value
   */
  TriNodeWithValue(TrieNode &&trieNode, T value) {}
   ...
   */
  TriNodeWithValue(char key_char, T value) {}

  /**
   * @brief Destroy the Trie Node With Value object
   */
  ~TriNodeWithValue() override = default;
...
/**
 * Trie is a concurrent key-value stored. Each key is string and its corresponding
...
class Trie {
 ...
 public:
   ...
   * 2. If the terminal node is a TrieNode, then convert it into TriNodeWithValue by
   ...
   * TODO(p0): Add implementation
   *
   * @brief Remove key value pair from the tire.
   ...
   * is not nullptr, then type T if type matches.
   *
   * @param key Key used to traverse the trie and find correct node
   * @param success Whether GetValue is successful or not
   * @return Value of type T is type matches
   */
  template <typename T>
